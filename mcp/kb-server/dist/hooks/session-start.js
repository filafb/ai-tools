...
try {
    if (!isOnDefaultBranch(cwd))
        process.exit(0); // WIP branch — skip
    if (!shouldNotify(cwd))
        process.exit(0); // Already notified at this HEAD
    ...
    if (stale.length > 0) {
        ...
        console.log(`⚠️  KB: ${stale.length} page(s) may be stale — ${preview}${more}. Run /kb:status to review.`);
    }
